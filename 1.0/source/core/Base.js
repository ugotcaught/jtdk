...
JS = {
	...
	/**
     * Set package for a loader. The default loader is "JS.ClassLoader".
     * 
     * @method setPath
     * @param {Object} pkgs
     * @param {JS.Loader} loader:optional
     */
    setPackage: function(pkgs, loader){
    	var l = loader||JS.ClassLoader;
    	l.setPackage(pkgs);
	},
    /**
	 ...
	 *     },
	 *     eat: function(food){
	 *         JS.print('"'+ this.getName() + '" Cat eat some ' + food);
	 *     }
	 * });
     * 
	 * @method define
	...
	factory: function(className, args, instance, loader){
		...
		var ctor = clazz.klass.getSelf(), obj = null;
		if(instance) {
			obj = instance;
		}else{
			var f = function(){};
...
var processDomReady = null, _processDomReady = function(scope) {
	domLoaded = true;
	if (supportSE) {
		doc.removeEventListener("DOMContentLoaded", processDomReady, false);
		doc.removeEventListener('load', processDomReady, false);
	}else {
		doc.detachEvent('onload', processDomReady);
	}
...
JS.main = function(fn, scope){
	if (domLoaded) {
		fn.call(scope);
		return;
	}

	processDomReady = function(){
		_processDomReady.call(scope);
	}
	
	if (supportSE) {
		doc.addEventListener("DOMContentLoaded", processDomReady, false);
	}else if (notIE9) {
		explorerTimer = win.setInterval(function(){
			if (doc.body) {
				try {
					createEl('div').doScroll('left');
					win.clearInterval(explorerTimer);
				} catch(e) {
					return;
				}

				_processDomReady(scope);
				return;
			}
		}, 10);
		...
		if (supportSE) {
			doc.addEventListener('load', processDomReady, false);
		} else {
			doc.attachEvent('onload', processDomReady);
		}
